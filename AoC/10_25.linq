...
async Task Main()
{
	...
	var schemas = lines.Select(i => 
		i.Split(']')[0].Substring(1).Select(i => i == '#').ToArray()
	).ToArray()
	//.Dump()
	;
	var buttonsRows = lines.
		Select(i =>
			i.Split(']')[1]
			.Split('{')[0]
			.Trim()
			...
			.Select(i => i.Substring(1).Split(',').Select(Int32.Parse).ToArray())
			.OrderByDescending(i => i.Length)
			.ToArray()
		).ToArray()
		//.Dump()
		;
	var counterRows = lines.
		Select(i =>
			i.Split('{')[1]
			.Trim()
			.Replace("}", "")
	...
	{
		var minPresses = RecursivePress(buttonsRows[row], 0, schema, new bool[schema.Length], 0);
		row++;
		
		if(minPresses == 99) throw new ApplicationException("No solution!");
	...
	foreach(var counterRow in counterRows)
		ProccessRow(counterRow, row++);
	
	//await Task.WhenAll(counterRows.Select((i, ind) => Task.Run(() => ProccessRow(i, ind))).ToArray());
	
	var curList = new List<int>();
	var lockObj = new Object();
	
	/*await Parallel.ForEachAsync(counterRows.Select((row, ind) => (row, ind)), 
	    new ParallelOptions { MaxDegreeOfParallelism = 12 },
	    async (item, ct) => ProccessRow(item.row, item.ind));*/
	
	void ProccessRow(int[] counterRow, int row)
	{
		//lock(lockObj){curList.Add(row); dc.Content = curList.ToArray();}
		
		var resultRec = new Solver().RecursivePress2(buttonsRows[row], counterRow, 0, new int[buttonsRows[row].Length]);
		if(!resultRec.isValid) throw new ApplicationException($"Invalid {row}!");
		result2 += resultRec.pressCt;
		row++;
		
		resultRec.pressCt.Dump($"{row}/{buttonsRows.Length}");
		//lock(lockObj){curList.Remove(row); dc.Content = curList.ToArray();}
	}
	
	(result2 == 0 ? "Invalid" : result2.ToString()).Dump("---------------------------2");
}

static DumpContainer dc = new DumpContainer();

int RecursivePress(int[][] buttons, int index, bool[] schema, bool[] current, int pressCt)
{
	var apply = current.ToArray();
	foreach(var flip in buttons[index])
		apply[flip] = !apply[flip];
	
	//Util.WithStyle(ShowSchema(apply).Dump($"{index} {ShowSchema(current)}| {pressCt}"), "font-family: monospace;");
	
	if(schema.SequenceEqual(apply))
	if(index + 1 == buttons.Length)
		return pressCt + 1;
	else
		return Math.Min(RecursivePress(buttons, index + 1, schema, current, pressCt), pressCt + 1);
	
	if(index + 1 == buttons.Length)
		return 99;
	
	return Math.Min(RecursivePress(buttons, index + 1, schema, current, pressCt), RecursivePress(buttons, index + 1, schema, apply, pressCt + 1));
}

class Solver
{
	private int curMinPressCt = Int32.MaxValue;
	private Dictionary<string, (bool isValid, int pressCt)> memory = new();
	
	public (bool isValid, int pressCt) RecursivePress2(int[][] buttons, int[] counters, int pressCt, int[] pressesStart)
	{
		if(curMinPressCt <= pressCt + 1) return (false, 0);
			
		var minCts = new List<int>();
		...
		foreach(var counter in counters)
		{
			...
			var curMin = counters[minCts.First()];
			if(counter < curMin)
			{
				minCts.Clear();
				minCts.Add(i);
			}
			else if(counter == curMin)
			{
				minCts.Add(i);
			}
			
			//ShowArray(minCts).Dump();
			
			i++;
		}
		
		//ShowArray(minCts).Dump(ShowArray(counters));
		
		var buttonsForMinCts = buttons.Select((i, ind) => (i, ind)).Where(i => i.Item1.Any(ii => minCts.Contains(ii)));
		
		var minPressCt = -1;
		foreach(var button in buttonsForMinCts)
		{
			int[] presses = new int[pressesStart.Length];
			Buffer.BlockCopy(pressesStart, 0, presses, 0, pressesStart.Length * sizeof(int));
			presses[button.Item2] += 1;
			var pressesString = String.Join(',', presses);
			
			if(memory.ContainsKey(pressesString))
			{
				var resultMem = memory[pressesString];
				if(resultMem.isValid) return resultMem;
				else continue;
			}
			
			var countersCopy = counters.ToArray();
			foreach(var op in button.Item1) countersCopy[op] -= 1;
			
			//ShowArray(countersCopy).Dump(ShowArray(counters));
			
			if(countersCopy.All(i => i == 0))
			{
				curMinPressCt = Math.Min(curMinPressCt, pressCt + 1);
				return (true, pressCt + 1);
			}
			if(countersCopy.Any(i => i < 0)) continue;
			
			var result = RecursivePress2(buttons, countersCopy, pressCt + 1, presses);
			
			memory.Add(pressesString, result);
			
			if(!result.isValid) continue;
			
			minPressCt = minPressCt == -1 ? result.pressCt : Math.Min(minPressCt, result.pressCt);
		}
		
		//if(minPressCt != -1) {var ct = minPressCt == -1 ? "invalid" : minPressCt.ToString(); dc.Content = $"{ct}, {pressCt} | cur min: {curMinPressCt}";}
		
		if(minPressCt == -1) return (false, 0);
		
		return (true, minPressCt);
	}
}

static string ShowSchema(bool[] current) =>
	String.Join("", current.Select(i => i ? "#" : "."));
static string ShowArray(IEnumerable<int> current) =>
	String.Join(",", current);
...
string raw = """
[.##......] (0,1,3,4,6,7,8) (1,2,3,5,6,8) (0,1) (3,5,6,7) (2,5,7) (1,2,3,4,5,7,8) (7) (0,1,3) (0,3,7) (1,4,6) {36,63,29,56,28,48,43,52,23}
